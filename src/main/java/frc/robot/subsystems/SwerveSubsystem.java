...
public class SwerveSubsystem extends SubsystemBase {
  ...
  public void drive(double xSpeed, double ySPeed, double rot, boolean fieldRelative) {
    if (fieldRelative) {
      swerveDrive.driveFieldOriented(new ChassisSpeeds(xSpeed, ySPeed, rot));
    } else {
      swerveDrive.setChassisSpeeds(new ChassisSpeeds(xSpeed, ySPeed, rot));
    }
  }

  public Command driveWithLimelight() {
    return this.run(
      () -> this.drive(
        LimelightHelpers.getTY("limelight") * -0.04, 
        0.0, 
        LimelightHelpers.getTX("limelight") * -0.02, false));
  }
...
// public Command goTagRelative(int tagId, double xMeters, double yMeters, double headingDeg) {
//   return Commands.defer(() -> {
//     if (!seesTag(tagId)) {  // Runtime check ✓
...
//       return Commands.none();
//     }
//     Pose2d currentPose = est.pose;  // Fixed Pose2d
//     Pose2d target = currentPose.transformBy(new Transform2d(xMeters, yMeters, Rotation2d.fromDegrees(headingDeg)));
//     return goToPose(target);
//   }, Set.of(this));  // Schedules properly [web:81]
//}
