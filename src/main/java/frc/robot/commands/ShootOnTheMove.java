 ...
 * Largely based off https://blog.eeshwark.com/robotblog/shooting-on-the-fly
 * Uses Limelight tx directly for turret angle and ty for distance calculation.
 */
public class ShootOnTheMove extends Command {
    ...
    // private static final double kLimelightHeightInches = 24.0;  // height of limelight lens from floor
    // private static final double kTargetHeightInches    = 60.0;  // height of target center from floor
    // private static final double kLimelightAngleDegrees = 30.0;  // limelight mounting angle from horizontal

    private final InterpolatingDoubleTreeMap shooter_table_ = new InterpolatingDoubleTreeMap();

    public ShootOnTheMove(Turret turret, SwerveSubsystem drivebase, VisionSubsystem vision, Pose2d goalPose) {
        turret_    = turret;
    ...
    public void execute() {
        ...
        Translation2d future_pos = drivebase_.getSwerveDrive().getPose().getTranslation().plus(
        ...
        if (vision_.hasTag()) {
            ...
            Translation2d robot_vel_vec = new Translation2d(
                robot_speed.vxMetersPerSecond,
                robot_speed.vyMetersPerSecond
            );
            ...
            Translation2d shot_vec = new Translation2d(
                Math.cos(tx_radians) * ideal_flywheel_percent * kTotalExitVelocity,
            ...
            Translation2d shot_vec = target_vec.div(dist)
                                               .times(ideal_flywheel_percent * kTotalExitVelocity)
                                               .minus(robot_vel_vec);
    ...
    /**
     * Calculate distance to target in meters using Limelight ty
     * Based on: distance = (targetHeight - limelightHeight) / tan(limelightAngle + ty)
     */
